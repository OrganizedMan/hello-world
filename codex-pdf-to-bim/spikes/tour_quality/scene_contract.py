"""The sole printed-dimension contract for the HearthView tour-quality spike."""
...
SPAN_METERS = 9.1694
ROOM_DEPTH_METERS = 4.8514
COUNTER_ZONE_DEPTH_METERS = 0.6604
CEILING_HEIGHT_METERS = 2.5654
ISLAND_WIDTH_METERS = 2.6162
ISLAND_DEPTH_METERS = 1.2954
WEST_CLEARANCE_METERS = 1.0668
NORTH_CLEARANCE_METERS = 1.0668
SOUTH_TRANSITION_METERS = 1.8288
LIVING_CLEAR_WIDTH_METERS = 4.4958
...
_REQUIRED_ORDERS = (
    ...
    ("east_south_transitions", ("mudroom_opening", "tv_wall", "south_living_opening")),
)
_REQUIRED_OPENINGS = (
...
def build_scene_contract() -> SceneContract:
    """Build the isolated, A-1-led scene contract consumed by later spike tasks."""
    envelope_depth = ROOM_DEPTH_METERS
    island_min_x = round(COUNTER_ZONE_DEPTH_METERS + WEST_CLEARANCE_METERS, 4)
    island_min_y = round(COUNTER_ZONE_DEPTH_METERS + NORTH_CLEARANCE_METERS, 4)
    island = Rectangle(
        "island",
        island_min_x,
        island_min_y,
        round(island_min_x + ISLAND_WIDTH_METERS, 4),
        round(island_min_y + ISLAND_DEPTH_METERS, 4),
    )
    living_start = SPAN_METERS - LIVING_CLEAR_WIDTH_METERS
    return SceneContract(
        schema=SCHEMA,
        label=SCENE_LABEL,
        canonical_geometry=False,
        envelope=Bounds(0.0, 0.0, 0.0, SPAN_METERS, envelope_depth, CEILING_HEIGHT_METERS),
        counter_zone_depth=COUNTER_ZONE_DEPTH_METERS,
        printed_dimensions=tuple(PrintedDimension(*value) for value in _PRINTED_DIMENSIONS),
        wall_openings=(
            WallOpening("kitchen_window_group", "north", Rectangle("kitchen_window_group", 3.95, 0.0, 5.30, 0.08)),
            WallOpening("deck_door_group", "north", Rectangle("deck_door_group", 6.05, 0.0, 8.80, 0.08)),
            WallOpening("mudroom_opening", "east", Rectangle("mudroom_opening", SPAN_METERS - 0.08, 0.45, SPAN_METERS, 1.45)),
            WallOpening("south_living_opening", "south", Rectangle("south_living_opening", living_start, envelope_depth - 0.08, 6.25, envelope_depth)),
        ),
        ...
        walkable_polygon=(
            (0.18, 0.18),
            (SPAN_METERS - 0.18, 0.18),
            (SPAN_METERS - 0.18, envelope_depth - 0.18),
            (0.18, envelope_depth - 0.18),
        ),
        collision_rectangles=(
            Rectangle("west_counter", 0.0, 0.0, COUNTER_ZONE_DEPTH_METERS, 2.75),
            Rectangle("north_counter", 0.0, 0.0, 3.70, COUNTER_ZONE_DEPTH_METERS),
            island,
            Rectangle("tv_wall", SPAN_METERS - 0.18, 1.25, SPAN_METERS, 2.80),
        ),
        camera_presets=(
            ...
            CameraPreset("overhead", (SPAN_METERS / 2, envelope_depth / 2, 8.0), (SPAN_METERS / 2, envelope_depth / 2, 0.0)),
        ),
